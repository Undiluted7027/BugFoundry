...
int ScenarioControl(int choice, int subchoice)
{
    switch (choice)
    {
        ...
        case 1:
            if (subchoice == 1) return NewCustomer();
            if (subchoice == 2) return CreateNewComplaint();
            if (subchoice == 3) return CreateNewProduct();
            if (subchoice == 4) return CreateNewProductRel();
            break;
        
        case 2:
            if (subchoice == 1) return UpdateSpecificChange();
            if (subchoice == 2) return ListAndSelectChange();
            break;
        
        case 3:
            if (subchoice == 1) return DisplayChangeReport();
            if (subchoice == 2) return ProductOnChange();
            if (subchoice == 3) return UserOnChange();
            break;
...
int DisplayChangeReport()
{
    ...
    std::cout << "CHANGE REPORT" << std::endl << std::endl;
...
int ListAndSelectChange()
{
    ...
    std::cout << "LATEST CHANGES" << std::endl << std::endl;
    ...
    do 
    {
        ...
        if (choice > 0 && choice <= (end - start)) {
            std::streampos selectedPos = static_cast<std::streampos>(CHANGEFILEPOINTER + (choice - 1) * sizeof(Change));
