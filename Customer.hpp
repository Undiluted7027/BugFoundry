...
class Customer
{
    public:
        ...
        ~Customer();
        /* 
        ~Customer() is the destructor for the class and does not have anything in it 
        since nothing has been assigned/created on the heap memory.
        ----------------------------------------------------------------------*/
        void DisplayDetails(
            std::ostream &out   // in out: ostream object to handle calling with << operator
        ) const;
        /* 
...
checks if input is valid by calling ValidateCustomer and returns a customer object. 
If the input is invalid, a message is displayed.
----------------------------------------------------------------------*/
void CommitCustomer(
    const Customer &customer,       // in: Customer to add in binary file
    std::streampos &startPos,       // in out: File pointer of the binary file
    const std::string &FILENAME     // in: Location-name of file
);
/* 
void CommitCustomer(const Customer &customer, std::startpos &startPos, const std::string &FILENAME) takes in a customer object, 
...
/* 
void PrintAllCustomers(const std::string &FILENAME) takes in location of binary file and 
...
/* 
int ValidateCustomer(const char *name, const char *email, const char *phone) is a function used 
for validating the inputs provided by the user i.e; Customer Support Person or a Software Developer. 
...
to check if binary file "Customers.bin" exist in the DIRECTORY to essentially check if the program is being run for the first time. 
